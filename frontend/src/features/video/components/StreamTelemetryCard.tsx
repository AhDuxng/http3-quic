...
export function StreamTelemetryCard({ stats, isPlaying, compact = false }: StreamTelemetryCardProps) {
  const items = useMemo(() => [
    ...
    { label: "GOODPUT", value: formatBitrateKbps(stats.goodputKbps), accent: false },
    { label: "OVERHEAD", value: `${(stats.overheadRatio * 100).toFixed(2)}%`, accent: stats.overheadRatio > 0.15 ? "red" : false },
    ...
    { label: "LOSS PROXY", value: `${(stats.lossProxyRate * 100).toFixed(2)}%`, accent: stats.lossProxyRate > 0 ? "red" : false },
