...
export function StreamTelemetryCard({ stats, isPlaying, compact = false }: StreamTelemetryCardProps) {
  const items = useMemo(() => [
    ...
    { label: "AVG BITRATE", value: formatBitrateKbps(stats.averageBitrateKbps), accent: "blue" },
    { label: "THROUGHPUT", value: formatBitrateKbps(stats.avgThroughputKbps), accent: "blue" },
    { label: "PAYLOAD RATE", value: formatBitrateKbps(stats.goodputKbps), accent: false },
    { label: "RESOURCE O/H", value: `${(stats.overheadRatio * 100).toFixed(2)}%`, accent: stats.overheadRatio > 0.15 ? "red" : false },
    ...
    { label: "FROZEN", value: String(stats.frozenFrameCount), accent: stats.frozenFrameCount > 0 ? "red" : false },
    { label: "STARTUP", value: stats.startupDelayMs > 0 ? `${stats.startupDelayMs} ms` : "—", accent: stats.startupDelayMs > 2000 ? "red" : false },
    { label: "TTFB", value: `${stats.ttfbMs.toFixed(1)} ms`, accent: stats.ttfbMs > 500 ? "red" : false },
    { label: "SDT VARIATION", value: `${stats.jitterMs.toFixed(1)} ms`, accent: stats.jitterMs > 100 ? "red" : false },
    ...
    { label: "FAIL/ABANDON", value: `${(stats.lossProxyRate * 100).toFixed(2)}%`, accent: stats.lossProxyRate > 0 ? "red" : false },
    ...
    { label: "Q.SWITCHES", value: `${stats.qualitySwitchCount} (${stats.qualityUpSwitchCount}↑/${stats.qualityDownSwitchCount}↓)`, accent: stats.qualityDownSwitchCount > 0 ? "red" : false },
