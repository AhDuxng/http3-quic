...
export function StreamTelemetryCard({ stats, isPlaying, compact = false }: StreamTelemetryCardProps) {
  const items = useMemo(() => [
    { label: "RESOLUTION", value: stats.resolutionLabel, accent: false },
    { label: "RENDERED BITRATE", value: formatBitrateKbps(stats.bitrateKbps), accent: "blue" },
    { label: "AVG VIEWED BITRATE", value: formatBitrateKbps(stats.averageBitrateKbps), accent: "blue" },
    { label: "THROUGHPUT", value: formatBitrateKbps(stats.avgThroughputKbps), accent: "blue" },
    { label: "PAYLOAD RATE", value: formatBitrateKbps(stats.payloadRateKbps), accent: false },
    ...
    { label: "FREEZE EVENTS", value: String(stats.freezeEventCount), accent: stats.freezeEventCount > 0 ? "red" : false },
    { label: "STARTUP→FRAME", value: stats.startupDelayMethod !== "not-measured" ? `${stats.startupDelayToFirstFrameMs} ms` : "—", accent: stats.startupDelayToFirstFrameMs > 2000 ? "red" : false },
    { label: "TTFB", value: `${stats.ttfbMs.toFixed(1)} ms`, accent: stats.ttfbMs > 500 ? "red" : false },
    { label: "SDT VARIATION", value: `${stats.segmentDownloadTimeVariationMs.toFixed(1)} ms`, accent: stats.segmentDownloadTimeVariationMs > 100 ? "red" : false },
    ...
    { label: "REQ FAIL/ABANDON", value: `${(stats.fragmentFailureOrAbandonRate * 100).toFixed(2)}%`, accent: stats.fragmentFailureOrAbandonRate > 0 ? "red" : false },
    ...
    { label: "RENDERED Q.SWITCH", value: `${stats.qualitySwitchCount} (${stats.qualityUpSwitchCount}↑/${stats.qualityDownSwitchCount}↓)`, accent: stats.qualityDownSwitchCount > 0 ? "red" : false },
    { label: "SETUP", value: `${stats.connectionSetupMs.toFixed(1)} ms`, accent: false },
  ...
  return (
    <div className="rounded-lg border border-slate-200 overflow-hidden bg-white">
      <div className={`${compact ? "px-3 py-2" : "px-4 py-2"} bg-slate-700 flex items-center justify-between`}>
      ...
      <div className="overflow-x-auto">
        <div className={`grid grid-flow-col ${compact ? "auto-cols-[6.75rem]" : "auto-cols-[7.5rem]"} divide-x divide-slate-100 w-max min-w-full`}>
          {items.map(({ label, value, accent }) => (
            <div key={label} className={compact ? "px-3 py-2.5" : "px-4 py-3"}>
